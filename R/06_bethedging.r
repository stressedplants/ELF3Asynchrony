...
s2ggplot(data=stressData[which(!is.na(stressData$Green.yellow)),], aes(x=as.numeric(Initalweight), y=as.numeric(weightChange), shape=Genotype, colour=Green.yellow))+geom_point()+theme_minimal()+xlab("Initial fresh weight (g)")+ylab("Change in fresh weight (g)")+
